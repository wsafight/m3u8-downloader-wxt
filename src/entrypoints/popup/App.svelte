<script lang="ts">
  ...
  async function refresh() {
    try {
      const res = await chrome.runtime.sendMessage({ type: 'GET_STREAMS', tabId });
  ...
  async function removeStream(url: string) {
    await chrome.runtime.sendMessage({ type: 'REMOVE_STREAM', tabId, url });
    streams = streams.filter(s => s.url !== url);
  }

  async function clearAll() {
    await chrome.runtime.sendMessage({ type: 'CLEAR_STREAMS', tabId });
    streams = [];
  }
  ...
  async function addToQueue(url: string) {
    ...
    await chrome.runtime.sendMessage({ type: 'ENQUEUE' });
  ...
  function parseUrl(raw: string): { host: string; path: string } {
    ...
    } catch { return { host: '', path: raw.slice(0, 45) }; }
  }
...
<div class="app">
  ...
  <nav class="tabs">
    ...
    <button class="tab" class:active={activeTab === 'history'} onclick={() => switchTab('history')}>历史记录</button>
  </nav>
  ...
  {#if activeTab === 'streams'}
        ...
        <div class="empty">
          <div class="empty-icon">
            <svg viewBox="0 0 64 64" fill="none">
              <circle cx="32" cy="32" r="28" stroke="var(--border-hi)" stroke-width="2" stroke-dasharray="6 4"/>
              <path d="M20 32a12 12 0 0124 0" stroke="var(--accent)" stroke-width="2.5" stroke-linecap="round"/>
              <path d="M14 32a18 18 0 0136 0" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" opacity=".5"/>
              <path d="M8 32a24 24 0 0148 0" stroke="var(--accent)" stroke-width="1.5" stroke-linecap="round" opacity=".25"/>
    ...
    <footer>
      ...
      <button class="btn-go" onclick={startManual} disabled={!manual.trim()}>
...
<style>
  .app {
    ...
    min-height: 200px; max-height: 580px;
    overflow: hidden;
  }
  ...
  .icon-btn:not(:disabled):hover { background: var(--surface-3); color: var(--error); }
  ...
  /* ── Main content ── */
  main { flex: 1; overflow-y: auto; min-height: 80px; }

  .center { display: flex; align-items: center; justify-content: center; padding: 40px; }
  .spinner {
    width: 28px; height: 28px;
    border: 2.5px solid var(--border-hi); border-top-color: var(--accent);
    border-radius: 50%; animation: spin .8s linear infinite;
  }

  .empty {
    display: flex; flex-direction: column; align-items: center;
    justify-content: center; padding: 32px 20px; gap: 10px;
  }
  .empty-icon svg { width: 64px; height: 64px; animation: pulse 3s ease-in-out infinite; }
  .empty-title { font-size: 13px; font-weight: 600; color: var(--text-2); }
  .empty-hint  { font-size: 11px; color: var(--text-3); }

  .stream-list { list-style: none; padding: 6px; display: flex; flex-direction: column; gap: 4px; }
  ...
  .stream-thumb svg { width: 16px; height: 16px; }

  .stream-info { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
  .stream-host { font-size: 12px; font-weight: 600; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .stream-path { font-size: 11px; color: var(--accent); opacity: .8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-family: 'SFMono-Regular', 'Consolas', monospace; }
  .stream-time { font-size: 10px; color: var(--text-3); }

  .stream-actions { display: flex; gap: 4px; flex-shrink: 0; }

  .btn-dl, .btn-queue, .btn-rm {
    display: flex; align-items: center; justify-content: center;
    width: 28px; height: 28px; border-radius: 6px;
    transition: background var(--transition), color var(--transition), transform .1s;
  }
  .btn-dl svg, .btn-queue svg, .btn-rm svg { width: 14px; height: 14px; }

  .btn-dl { background: var(--accent-glow); color: var(--accent); border: 1px solid var(--border-hi); }
  .btn-dl:hover { background: var(--accent); color: #fff; transform: translateY(-1px); }
  .btn-dl:active { transform: scale(.92); }

  .btn-queue { background: transparent; color: var(--text-3); border: 1px solid transparent; }
  .btn-queue:hover { background: #5b9df615; color: var(--accent); border-color: var(--border-hi); }

  .btn-rm { background: transparent; color: var(--text-3); border: 1px solid transparent; }
  .btn-rm:hover { background: #f871711a; color: var(--error); border-color: #f8717130; }
  ...
  .input-icon { width: 14px; height: 14px; color: var(--text-3); flex-shrink: 0; }
  .url-input { flex: 1; background: transparent; border: none; color: var(--text); font-size: 12px; padding: 7px 0; outline: none; }
  ...
  .btn-go:not(:disabled):hover { opacity: .85; transform: translateY(-1px); }
  .btn-go:not(:disabled):active { transform: scale(.92); }
