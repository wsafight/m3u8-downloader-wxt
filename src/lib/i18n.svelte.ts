...
const zh = {
  ...
  // ── Downloader messages ──────────────────────────────────────────
  dlFetchingPlaylist: '正在获取播放列表…',
  dlNoStreamsInMaster: 'Master playlist 中没有可用流',
  dlFetchingStream: (label: string) => `获取媒体流 (${label})…`,
  dlNoSegmentsInMedia: '媒体播放列表中没有分片',
  dlFetchingInitSegment: '获取初始化分片…',
  dlSegmentInfo: (total: number, fmt: string, dur: string) => `共 ${total} 个分片 (${fmt})，时长约 ${dur}，开始并发下载…`,
  dlRestoredFromCache: (count: number) => `已从缓存恢复 ${count} 个分片，继续下载剩余分片…`,
  dlSegmentFailed: (index: number) => `分片 ${index} 下载失败`,
  dlSomeSegmentsFailed: (count: number) => `${count} 个分片失败，可点击"重新下载失败片段"重试`,
  dlNoFailedSegments: '没有失败的分片',
  dlRetryingFailed: (count: number) => `重试 ${count} 个失败分片…`,
  dlSegmentRetryFailed: (index: number) => `分片 ${index} 重试失败`,
  dlStillFailed: (count: number) => `仍有 ${count} 个分片失败`,
  dlRetryComplete: '重试完成，正在合并…',
  dlMerging: '正在合并分片…',
  dlNoOkSegments: '没有已成功的分片',
  dlSavingPartial: (count: number) => `正在保存 ${count} 个已完成分片…`,
  dlSavedPartial: (count: number, mb: string) => `已保存 ${count} 个分片（${mb} MB）`,
  dlConvertingMp4: '正在转换为 MP4…',
  dlMp4ConvertDone: 'MP4 转换完成',
  dlMp4ConvertFailed: (msg: string) => `MP4 转换失败（${msg}），将保存为 .ts`,
  dlPrepareSave: '准备保存文件…',
  dlDownloadComplete: (segs: number, mb: string) => `下载完成！${segs} 个分片，${mb} MB`,
  dlNetworkError: (url: string) => `网络请求失败，请检查连接（${url}）`,
  dlHttp403: (url: string) => `403 拒绝访问 — 该流可能需要登录或 Referer，请刷新视频页面后重新触发下载（${url}）`,
  dlHttp404: (url: string) => `404 未找到 — 链接已失效或分片已过期（${url}）`,
  dlHttp429: (url: string) => `429 请求过于频繁 — 尝试降低并发数后重试（${url}）`,
  dlHttpServer: (status: number, url: string) => `${status} 服务器错误 — 稍后重试（${url}）`,
  dlHttpGeneric: (status: number, url: string) => `HTTP ${status}（${url}）`,
  dlAes128MissingKey: 'AES-128 加密但缺少密钥 URI',
  dlCircuitOpen: (n: number) => `连续 ${n} 个分片失败，已自动中止下载以防止无效重试`,

  // ── LiveRecorder messages ────────────────────────────────────────
  lrStartRecording: '开始录制直播流…',
  lrPlaylistFailed: (status: number) => `播放列表请求失败：HTTP ${status}，3 秒后重试`,
  lrFetchFailed: (msg: string) => `拉取播放列表失败：${msg}，3 秒后重试`,
  lrStreamEnded: '直播流已结束',
  lrSegmentFailed: (err: string, url: string) => `分片下载失败（${err}），已跳过: ${url}`,
  lrMergingRecording: '正在合并录制内容…',
  lrRecordingDone: (count: number, mb: string) => `录制完成！${count} 片，${mb} MB`,
  lrAes128MissingKey: 'AES-128 加密但缺少密钥 URI',
  lrKeyFetchFailed: (status: number) => `密钥请求失败：HTTP ${status}`,
